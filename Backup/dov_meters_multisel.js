...
var createMeterGrid = function(id){ 
  ...
  jQuery('#dov_meters_table').jqGrid({
    ...
    scroll: 0,
    scrollrows : true,
    colNames:['Код','Назва','ГОСТ', 'Напруга ном.','Ток ном.','Тип','Фазність','Розр.','Зон','Мін. тривалісь зони','Пер.повірки','Клас точності','Порог чутливості','АЕ','РЕ','АЕг','РЕг','*'],
    colModel :[ 
      ...
      {name:'zone_time_min', index:'zone_time_min', width:80, editable: true, align:'right',hidden:true,
                            edittype:'text',formatter:'number',editrules:{number:true,edithidden:true} },           
      {name:'term_control', index:'term_control', width:80, editable: true, align:'right',hidden:false,
                            edittype:'text',formatter:'integer',editrules:{required:true,number:true,edithidden:true} },           
      ...
      {name:'show_def', index:'show_def', width:30, editable: true, align:'right',hidden:true,
...
jQuery("#dov_meters_table").jqGrid('bindKeys', {"onEnter":function( id ) { 
    ...
    $("#t_dov_meters_table").append("<button class ='btnOk' id='bt_metsel_all' style='height:20px;font-size:-3' > Выбор несколько </button> ");
...
jQuery('#bt_metsel_all').click( function() { 
    
    var ids =$("#dov_meters_table").jqGrid('getGridParam','selarrrow');
    var json_str = JSON.stringify(ids); 
    
    meter_target_id.val( '' ); 
    meter_target_name.val('- Декілька типів -' ); 
    meter_target_id_array.val( json_str ); 
    meter_target_name.focus();
    meter_target_name.focus();
