  ...
  jQuery('#dov_corde_table').jqGrid({
    ...
    colModel :[ 
      ...
      {name:'materal', index:'materal', width:100, editable: true, align:'right',
                            edittype:'select',formatter:'select',editoptions:{value:lmateral} },                       
...
$("#debug_ls3").click( function() {jQuery("#message_zone").html('');});
 ...
 outerLayout = $("body").layout({
	...
	,       center__onresize:	function (pane, _pane, state, options) 
        {
            jQuery("#dov_corde_table").jqGrid('setGridWidth',_pane.innerWidth()-9);
            jQuery("#dov_corde_table").jqGrid('setGridHeight',_pane.innerHeight()-110);
        }
