...
$print_mode=1;   // 0 - 3 to page, 1 - free mode
...
{   
    ...
    else
    {
        ...
        if ($mode == 3)
        {
            $cstr="host=$app_host dbname=$app_main_db user=$app_user password=$app_pass"; 
            //echo $cstr;
            if (!($new_link = pg_connect($cstr))) 
            {
...
$Link = get_database_link($_SESSION,3);
//$session_user = $_SESSION['ses_usr_id'];
session_write_close();


$nmp1 = "АЕ-Друк рахунку";
...
$id_res = $row['id_res'];  
 ...
 else
 {
   if ((isset($_GET['id_bill']))&& ($_GET['id_bill']!=''))     
   {
    ...
    $where = " and (b.idk_doc = 200 or (b.idk_doc = 220 and b.demand >0)) and b.id_doc = $id_bill ";
    ...
    */
    ...
    $SQL = " select rs.id_region , coalesce(r.code_res,0) as id_res,
            b.mmgg, b.mmgg_bill, b.id_paccnt
...
<style type="text/css">
  .bill_class {page-break-inside: avoid; width:19.2cm; clear:both;}
</style>
...
round(s.e_val ,2) + COALESCE(bk2.value,0)+ COALESCE(bk3.value,0) as saldo_e, 
round(s.e_valtax,2) + COALESCE(bk2.value_tax,0) + COALESCE(bk3.value_tax,0) as e_valtax,
...
COALESCE(bk.value,0) + COALESCE(bk3.value,0) as korr_val,
...
left join ( select id_paccnt, mmgg, sum(value) as value from acm_bill_tbl where id_pref = 10 and idk_doc = 220
  group by id_paccnt, mmgg order by id_paccnt) as bk 
 on (bk.id_paccnt = b.id_paccnt and (bk.mmgg = b.mmgg and b.idk_doc = 200 or bk.mmgg = b.mmgg_bill and b.idk_doc = 220 )  )
...
left join ( select id_paccnt, sum(value) as value , sum(value_tax) as value_tax  
    from acm_bill_tbl 
    where id_pref = 10 and idk_doc = 220
    and mmgg = $mmgg and mmgg_bill < $mmgg_bill and id_paccnt = $id_paccnt
    group by id_paccnt) as bk3 
 on (bk3.id_paccnt = b.id_paccnt and b.idk_doc = 220 )
...
//    $total_value_tax =$saldo_e_tax+$avans_tax;
   ...
   // bill header !!!!
    ...
    <div class="bill_left_part" style="float:left; width:12.2cm"> 
    <p class="sinfo_h">Як передати показники лічильника</p>
     <table class="bill_info_table" width="100%" cellspacing="0"> 
        <tr>
            ...
            <td width="25%"><p class="sinfo">Автоматичний збір</p>
            ...
            <td width="25%" class ="last"><p class="sinfo">SMS-повідомлення</p>
     ...
     <table class="bill_left_table" width="465" cellspacing="0"> 
...
$bill_text.= <<<BILL_TABLEHDR_LEFT
        <tr>
            <td width="85" rowspan="2" valign="top"><span class="snorm">Дата показ.</span></td>
            <td colspan="2"><span class="snorm">Показ. лічильника </span></td>
            <td width="49"><span class="snorm">Спожито</span></td>
            <td width="60"><span class="snorm">Вартість</span></td>
        ...
        <tr>
            ...
            <td width="80"><div align="center"><span class="snorm">попередні </span></div></td>
...
//    <p class="sbold">Адреса ІКЦ $ikc_addr</p>
  ...
  <div style="float:left ; width:6.4cm;margin-left:0.5cm">
...
$bill_text.= '<HR style="margin-top: 10px; clear:both;" SIZE="1" WIDTH="100%" NOSHADE/> ';
echo $bill_text;    
 /*   
 if ($print_mode == 0)
 {
