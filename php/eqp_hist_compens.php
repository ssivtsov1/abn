...
$fildsArray =DbGetFieldsArray($Link,'eqm_compensator_h');
...
$Query="SELECT COUNT(*) AS count FROM eqm_compensator_h $qWhere;";
...
$SQL = "select eq.id_key,eq.id_type_eqp, i.name as type_eqp,
       eq.dt_b, eq.dt_e, u1.name as user_name, eq.mmgg, eq.dt 
       from eqm_compensator_h as eq 
       left join eqi_compensator_tbl as i on (i.id = eq.id_type_eqp)
